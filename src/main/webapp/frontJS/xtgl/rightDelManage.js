...
$(function(){
	...
	/* obtain data */
	$.ajax({
		url:'../../authority/getRoleMenuPerms.svc ',
		type:'POST',
		data:JSON.stringify({
			roleCode:_roleCode
		}),
			...
			if(data.succ){
				...
				if(isArray(_a)){
					...
					if(_a.length != 0){
						...
						$('#search-err').text('暂无可供删除的菜单').fadeIn(300);
					}
...
$(function(){
	...
	$('#save-btn').on('click',function(){
         ...
         console.log(_allMenu)
         
         if(_allMenu.length == 0){
        	 $('#add-err').text('尚未勾选菜单，请勾选您需要删除的菜单~').fadeIn(300);
        	 return false;
         }
         
         //保存菜单配置
         $.ajax({
 			url:'../../authority/delRoleMenuPerms.svc',
 			type:'POST',
 			data:JSON.stringify({
 				roleCode:_roleCode,
 			...
 			error:function(){
 				$('#add-err').text('菜单删除保存失败，请重新尝试，或联系技术人员处理！谢谢~').fadeIn(300);
 			}
 		});
	});
