    ...
    var ajaxOne = new relaxAJAX({
        ...
        success:function(data){
        	...
        	if(data.succ){
                ...
                var tmpHtml = '<tr><th>序号</th><th>订单编号</th><th>充值时间</th><th>号码</th><th>流量值</th><th>状态</th></tr>';
        		...
        		if(data.result.total != 0){
                    ...
                    for(var i = 0;i < info.length;i++){
                    	switch(parseInt(info[i].status)){
	                		case 1:
	                			tmpHtml += '<tr><td>'+parseInt(parseInt(currentPage-1)*pageSize+i+1)+'</td><td>'+info[i].orderId+'</td><td>'+info[i].startTime+'</td><td>'+info[i].chargeNumber+'</td><td>'+(info[i].elem3)+'</td><td>充值中</td></tr>';
	                			break;
	                		case 2:
	                			tmpHtml += '<tr><td>'+parseInt(parseInt(currentPage-1)*pageSize+i+1)+'</td><td>'+info[i].orderId+'</td><td>'+info[i].startTime+'</td><td>'+info[i].chargeNumber+'</td><td>'+(info[i].elem3)+'</td><td>成功</td></tr>';
	                			break;
	                		case 3:
	                			tmpHtml += '<tr><td>'+parseInt(parseInt(currentPage-1)*pageSize+i+1)+'</td><td>'+info[i].orderId+'</td><td>'+info[i].startTime+'</td><td>'+info[i].chargeNumber+'</td><td>'+(info[i].elem3)+'</td><td>失败</td></tr>';
	                			break;
	                		case 4:
	                			tmpHtml += '<tr><td>'+parseInt(parseInt(currentPage-1)*pageSize+i+1)+'</td><td>'+info[i].orderId+'</td><td>'+info[i].startTime+'</td><td>'+info[i].chargeNumber+'</td><td>'+(info[i].elem3)+'</td><td>撤单中</td></tr>';
	                			break;
	                		default:
	                			tmpHtml += '<tr><td>'+parseInt(parseInt(currentPage-1)*pageSize+i+1)+'</td><td>'+info[i].orderId+'</td><td>'+info[i].startTime+'</td><td>'+info[i].chargeNumber+'</td><td>'+(info[i].elem3)+'</td><td>无</td></tr>';
	                			break;
	                	}
        			...
        			initStatus = 0;
        			$('.tableContainer table').empty().append('<tr><th>序号</th><th>订单编号</th><th>充值时间</th><th>号码</th><th>流量面值</th><th>状态</th></tr>');
