...
function ajaxDom(pageNum,pageSize,obj){
	...
	for(var i=0;i<obj.length;i++){
		...
		<td>'+(obj[i].ownershipNumber ? obj[i].ownershipNumber : "--")+'</td>\
		<td>'+(obj[i].proDate ? obj[i].proDate : "--")+'</td>\
		<td style="max-width:200px;max-height:50px;overflow-x:auto;line-height:20px;">'+obj[i].ownerName+'</td>\
		<td style="max-width:200px;max-height:50px;overflow-x:auto;line-height:20px;">'+obj[i].applicantName+'</td>\
		<td>'+obj[i].mobile+'</td>\
		<td>'+dealType[obj[i].transType]+'</td>\
		<td>'+isHang[obj[i].isOn]+'</td>';
		...
		_html += '<td><a href="javascript:toPage(\''+obj[i].id+'\')" class="underline-a look-a">查看详情</a></td>\
    	</tr>';
	}
