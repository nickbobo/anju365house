...
$.fn.searchbox.defaults=$.extend({},$.fn.textbox.defaults,{inputEvents:$.extend({},$.fn.textbox.defaults.inputEvents,{keyup:function(e){
if(e.keyCode==13){
e.preventDefault();
var t=$(e.data.target);
var opts=t.searchbox("options");
...
/* modify by relax 用于datagrid的加载提示的简易调用 */
(function(){
	$.extend($.fn.datagrid.methods,{
		//显示遮罩 
		loading: function(jq, setting){
			return jq.each(function(){
				$(this).datagrid("getPager").pagination("loading"); 
				var opts = $(this).datagrid("options"); 
				...
				if (setting){
					...
					showText = setting.msg;
				}
				if (showText == "") showText = opts.loadMsg.replace(/。/g,".");
				if (opts.loadMsg){
					if (wrap.find(".datagrid-mask").length == 0){
						$("<div class=\"datagrid-mask\"></div>").css({display:"block",width:wrap.width(),height:wrap.height()}).appendTo(wrap); 
					}
					if (wrap.find(".datagrid-mask-msg").length == 0){
						$("<div class=\"datagrid-mask-msg"+ classStr +"\"></div>").html(showText).appendTo(wrap).css({display:"block",left:(wrap.width()-$("div.datagrid-mask-msg",wrap).outerWidth())/2,top:(wrap.height()-$("div.datagrid-mask-msg",wrap).outerHeight())/2});
					}else{
						var temp = wrap.find(".datagrid-mask-msg").eq(0);
						temp.removeClass("ok").removeClass("error");
						if (classStr){
							temp.addClass(classStr.replace(" ",""));
						}
						temp.html(showText);
					}
				}
