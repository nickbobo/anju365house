...
SPAmenu.prototype.showItem = function(eventObj, domObj, txt, title, href){
	...
	if (domObj === null){
		...
		CL._show(key);
		if (typeof(eventObj)){
			js_protectEvent(eventObj);
		}
	}else{
		var key = domObj.attr("key");
...
//SPAmenu.prototype.test = function(){
	//var CL = this;
	/*
	var tempObj = {};
	for (var x in CL.menuList){
	...
	*/
	//console.log(CL.domBar[0].scrollLeft + " " + CL.domBar[0].scrollWidth);
	//console.log(CL.menuList);
//}
