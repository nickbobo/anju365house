	...
	for(var i=0;i<obj.length;i++){
		...
		for(var j=0;j<applicantNatureData.length;j++){
			...
			if(obj[i].proStatus == 0){
				...
				_html += '<td><a href="javascript:toPage(\''+obj[i].id+'\')" class="underline-a set-a">查看详情</a><a href="javascript:toPage(\''+obj[i].id+'\')" class="underline-a set-a">立即验证</a></td></tr>';
			}
			if(obj[i].proStatus == 1){
				_html += '<td class="pass">'+houseCheckStatus[obj[i].proStatus]+'</td>';
				_html += '<td><a href="javascript:toPage(\''+obj[i].id+'\')" class="underline-a set-a">查看详情</a></td></tr>';
			}
